...
static const char* _node_name(void) {
  return RocsWgen_weathercolor.name;
}
static const char* _node_remark(void) {
  return RocsWgen_weathercolor.remark;
}
static Boolean _node_required(void) {
  return RocsWgen_weathercolor.required;
}
static const char* _node_cardinality(void) {
  return RocsWgen_weathercolor.cardinality;
}
...
static int _getblue(iONode node) {
  ...
  xNode( RocsWgen_weathercolor, node );
 ...
 * sat
 */
static struct __attrdef RocsWgen_sat = {
  "sat",  "",  "",  "int",  "200",  "0-255",  False,
};
static int _getsat(iONode node) {
  int defval = xInt( RocsWgen_sat );
  
  if( node == NULL ) {
    return defval;
  }
  xNode( RocsWgen_weathercolor, node );
  return NodeOp.getInt( node, "sat", defval );
}
static void _setsat(iONode node, int p_sat) {
  if( node == NULL ) return;
  xNode( RocsWgen_weathercolor, node );
  NodeOp.setInt( node, "sat", p_sat );
}
...
static Boolean _node_dump(iONode node) {
  if( node == NULL && RocsWgen_weathercolor.required ) {
    TraceOp.trc( "param", TRCLEVEL_EXCEPTION, __LINE__, 9999, ">>>>> Required node weathercolor not found!" );
    return False;
  }
  else if( node == NULL ) {
    TraceOp.trc( "param", TRCLEVEL_WRAPPER, __LINE__, 9999, "Node weathercolor not found!" );
    return True;
  }
  ...
  attrList[3] = &RocsWgen_hour;
  attrList[4] = &RocsWgen_minute;
  attrList[5] = &RocsWgen_red;
  attrList[6] = &RocsWgen_sat;
  attrList[7] = &RocsWgen_white;
  attrList[8] = &RocsWgen_white2;
  attrList[9] = NULL;
  nodeList[0] = NULL;
  {
    int i = 0;
...
struct __wWeatherColor wWeatherColor = {
  _node_name,
  _node_remark,
  _node_required,
  _node_cardinality,
