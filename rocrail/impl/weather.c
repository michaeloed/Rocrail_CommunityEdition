          ...
          if( cnt == wWeatherTheme.getsoundrandomrate(data->theme) && StrOp.len(wWeatherTheme.getsound(data->theme)) > 0 ) {
            if( data->themesoundtimer <= 0 ) {
              if( StrOp.len(wWeatherTheme.getsound(data->theme)) > 0 ) {
                char* s = StrOp.fmt("%s \"%s%c%s\"", wFreeRail.getsoundplayer(AppOp.getIni()),
                    wFreeRail.getsoundpath(AppOp.getIni()), SystemOp.getFileSeparator(), wWeatherTheme.getsound(data->theme) );
...
static void __makeWeather( void* threadinst ) {
  ...
  while( data->run ) {
    ...
    if( loopCnt >= 10 ) {
      ...
      if( lastMin != min ) {
        if( wCtrl.isweather(wFreeRail.getctrl(AppOp.getIni())) ) {
