...
typedef struct _gtodTimezone
{
    int minutesWest;
    int dstTime;
}gtodTimezone, *PgtodTimezone;

int gettimeofday(struct timeval* tv, PgtodTimezone tz)
{
    FILETIME ft;
    ...
    if (tz)
    {
        ...
        tz->minutesWest = _timezone / 60;
        tz->dstTime = _daylight;
    }
    return 0;
}
#endif
...
static void __manager( void* threadinst ) {
  ...
  do {
    ...
    if( client != NULL ) {
      ...
      SrcpService = ThreadOp.inst( servername, __SrcpService, cargo );
      divider     = wClock.getdivider(wFreeRail.getclock(AppOp.getIni()));
...
static struct OSrcpCon* _inst( iONode ini, srcpcon_callback callbackfun, obj callbackobj ) {
  ...
  if( wFreeRail.ispoweroffonexit(AppOp.getIni()) ) {
