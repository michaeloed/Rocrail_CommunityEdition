...
static const char* _node_name(void) {
  return RocsWgen_sunset.name;
}
static const char* _node_remark(void) {
  return RocsWgen_sunset.remark;
}
static Boolean _node_required(void) {
  return RocsWgen_sunset.required;
}
static const char* _node_cardinality(void) {
  return RocsWgen_sunset.cardinality;
}
...
static Boolean _node_dump(iONode node) {
  if( node == NULL && RocsWgen_sunset.required ) {
    TraceOp.trc( "param", TRCLEVEL_EXCEPTION, __LINE__, 9999, ">>>>> Required node sunset not found!" );
    return False;
  }
  else if( node == NULL ) {
    TraceOp.trc( "param", TRCLEVEL_WRAPPER, __LINE__, 9999, "Node sunset not found!" );
    return True;
  }
