...
void HueConfDlg::event(iONode node) {
  ...
  // Lights
    ...
    for( int i = 0; i < cnt; i++ ) {
      ...
      m_LightsGrid->SetCellAlignment(row, 0, wxALIGN_CENTRE, wxALIGN_CENTRE);
      ...
      if( state != NULL ) {
        ...
        m_LightsGrid->SetCellAlignment(row, 3, wxALIGN_CENTRE, wxALIGN_CENTRE);
        m_LightsGrid->SetReadOnly( row, 3, true );
        m_LightsGrid->SetCellValue(row, 4, wxString(NodeOp.getStr(state, "bri", "0"),wxConvUTF8) );
        m_LightsGrid->SetCellAlignment(row, 4, wxALIGN_CENTRE, wxALIGN_CENTRE);
